...
- **Home "流動的光與色" featured strip** → the `Home` `featured` array (art ids, top of `Home` in `pages.jsx`) is rendered as a fixed-height **mosaic** that assumes **exactly 6 items**: per-index `colSpan = [8,4,4,4,4,4]` and the first tile gets `rowSpan 2` (hero). Combined with `.featured-strip { grid-auto-rows; grid-auto-flow:dense }` in `styles.css`, this tiles a 12-col × 3-row block with no gaps (hero left, two stacked right, three across the bottom). Changing the count without updating those span arrays re-breaks the fill — keep it at 6, or rebalance the spans so each row sums to 12. Mobile (`max-width:560px`) overrides to a 2-col square grid (resets `grid-row`/`grid-auto-rows` and forces `aspect-ratio:1/1`).
